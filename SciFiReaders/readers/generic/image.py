...
"""
:class:`~SciFiReaders.generic.image.ImageTranslator` class that extracts the
...
class ImageReader(Reader):
    ...
    def read(self, **image_args):
        ...
        ext = os.path.splitext(self._input_file_path)[-1]
        original_metadata, metadata, dimensions = {}, {}, []
        if ext in ['jpg', 'jpeg', 'png', 'bmp']:
            img_data = np.asarray(PIL.Image.open(self._input_file_path))
            ...
            if tff is None:
                ...
                tif = tff.TiffFile(self._input_file_path)
