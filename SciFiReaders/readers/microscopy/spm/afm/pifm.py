...
class PiFMTranslator(Reader):
    ...
    def read(self, path ):
        ...
        self.datasets = [] #list of sidpy datasets
    ...
    def make_datasets(self):
        ...
        if bool(self.spectrogram_desc):
           ...
           for spectrogram_f, descriptors in self.spectrogram_desc.items():
               ...
               spectrogram_data = self.spectrograms[spectrogram_f].reshape((self.x_len * self.y_len, len(spec_vals_i)))
               dset = sid.Dataset.from_array(spectrogram_data)
               ...
               #                             (self.x_len * self.y_len, len(spec_vals_i)),  # shape of Main dataset
    ...
    def make_dimensions(self):
        ...
        self.dim0 = Dimension(self.x_len, name = 'x', units = qtyx, dimension_type = 'spatial')
        self.dim1 = Dimension(self.y_len, name = 'y', units = qtyy, dimension_type = 'spatial')
    ...
    def write_spectra(self):
        ...
        if bool(self.spectrum_desc):
            for spec_f, descriptors in self.spectrum_desc.items():
                ...
                spec_i_pos_dims = [Dimension('X',self.params_dictionary['XPhysUnit'].replace('\xb5','u'),
                                             np.array([float(descriptors[1])])),
                                   Dimension('Y',self.params_dictionary['YPhysUnit'].replace('\xb5','u'),
                                             np.array([float(descriptors[1])]))]
